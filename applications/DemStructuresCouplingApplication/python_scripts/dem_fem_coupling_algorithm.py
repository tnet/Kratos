    ...
    def RunSolutionLoop(self):
        ...
        while self.structural_solution.time < self.structural_solution.end_time:
            ...
            for self.dem_solution.time_dem in self.yield_DEM_time(self.dem_solution.time, time_final_DEM_substepping, self.Dt_DEM):
                ...
                self.dem_solution.time = self.dem_solution.time + self.Dt_DEM

                self.dem_solution.step += 1

                self.dem_solution.DEMFEMProcedures.UpdateTimeInModelParts(self.dem_solution.all_model_parts, self.dem_solution.time, self.Dt_DEM, self.dem_solution.step)
                ...
                DemFem.ComputeDEMFaceLoadUtility().CalculateDEMFaceLoads(self.skin_mp, self.Dt_DEM, self.Dt_structural)

                self.dem_solution.DEMFEMProcedures.MoveAllMeshes(self.dem_solution.all_model_parts, self.dem_solution.time, self.Dt_DEM)
